...
serve(async (req) => {
  ...
  try {
    ...
    const { name, dosage, frequency, conditions } = await req.json();
    if (!name || typeof name !== "string") {
      return new Response(JSON.stringify({ error: "Missing medication name" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const systemPrompt = `You are a friendly clinical pharmacist explaining medications to patients in plain, reassuring language. Respond ONLY with valid JSON, no markdown:
{
